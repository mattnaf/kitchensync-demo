...
import generateCartId from '@/functions/generateCartId';
...
const Menu = (props) => {
  ...
  const confirmPayment = async () => {
    ...
    let cartId = generateCartId(tableNumber)
    ...
    //   let cartId = generateCartId(tableNumber) + `${index}`
    //   await setDoc(doc(db, "orders", cartId), {
    //     table: tableNumber,
    //     title: item.title,
  ...
  return (
    <div className={styles.menuPage}>
      ...
      <PaymentModal hide={() => setShowPayment(false)} show={showPayment} items={cart} updateTip={(tip) => onTipChange(tip)} tip={tip} submitPayment={() => confirmPayment()}/>
